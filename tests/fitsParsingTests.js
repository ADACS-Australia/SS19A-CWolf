...
if (cluster.isMaster) {
    ...
    /*
    tests.addTest(new Test("FITS ELG\tlog multiarray vacuum wavelength, no sky, no helio, no cmb", function() {
        var q = $q.defer();
    ...
    tests.addTest(new Test("JSON ELG\tlog multiarray vacuum wavelength, no sky, no helio, no cmb", function() {
        var q = $q.defer();
        struct.runJSONFile("./testFits/emlLogVacuumHelioMultiple.json", null, debug, false).then(function(res) {
            var spectra0 = res[0];
            var spectra1 = res[1];
            var spectra2 = res[2];
            q.resolve([spectra0.getFinalRedshift(), spectra1.getFinalRedshift(), spectra2.getFinalRedshift()]);
