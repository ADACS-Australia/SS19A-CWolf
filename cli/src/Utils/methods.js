...
import {polynomial, polynomial3, getDataPowered} from '../Utils/regression';
 ...
 */
export function convertAirFromVacuumWithLogLambda(lambda) {
    for (var i = 0; i < lambda.length; i++) {
 ...
 * @param addition
 */
//export function add(original, addition) {
 ...
 * @returns {Array}
 */
export function linearScaleFactor(start, end, redshift, num) {
 ...
 * @returns {Array}
 */
export function getNewSubtract(data, subtract) {
...
export function applySpectralLineWeighting(lambda, spec) {
...
export function getList(linkedList) {
    var r = [];
...
export function addAllToSorted(head, values) {
...
export function subtractMeanReject(final, trimAmount) {
...
export function getQuasarFFT(lambda, intensity, variance) {
    ...
    let quasarResult = convertLambdaToLogLambda(lambda, quasarIntensity, globalConfig.arraySize, true);
    quasarIntensity = quasarResult.intensity;
    let quasarFFT = new FFT(quasarIntensity.length, quasarIntensity.length);
...
export function getStandardFFT(lambda, intensity, variance, needSubtracted) {
    ...
    if (needSubtracted) {
        let subtracted = intensity.slice();
    }
    ...
    let result = convertLambdaToLogLambda(lambda, intensity, globalConfig.arraySize, false);
    intensity = result.intensity;

    // Fourier transform both the intensity and quasarIntensity variables
    let fft = new FFT(intensity.length, intensity.length);
