...
(function() {
  ...
  window.astro.FITS.DataUnit = DataUnit;
  ...
  window.astro.FITS.HeaderVerify = HeaderVerify;
  ...
  Header = (function(_super) {
    ...
    Header.prototype.maxLines = 1600;
  ...
  window.astro.FITS.Header = Header;
  ...
  window.astro.FITS.ImageUtils = ImageUtils;
  ...
  Image = (function(_super) {
    ...
    Image.prototype._getFrame = function(buffer, bitpix, bzero, bscale) {
      ...
      if (bitpix > 0) {
        ...
        if (bitpix == -32) {
            ...
            arr = new Float64Array(buffer.byteLength / 8);
            for (var i = 0; i < arr.length; i++) {
              arr[i] = dv.getFloat64(i*8, false);
              arr[i] = bzero + bscale * arr[i];
            }
        }
      }
      return arr;
    ...
    Image.prototype._getFrameAsync = function(buffers, callback, opts) {
      var bitpix = this.bitpix;
      var bzero = this.bzero;
      var bscale = this.bscale;
      var start = 0;
      var pixels = new Float64Array(this.width * this.height);
      for (var i = 0; i < this.nBuffers; i++) {
        var buffer = buffers[i];
        var arr = this._getFrame(buffer, bitpix, bzero, bscale);
        pixels.set(arr, start);
        start += arr.length;
      }
      var fn = function() {
        callback(pixels, opts);
      };
      setTimeout(fn, 1);
    };
    /*
    Image.prototype._getFrameAsync2 = function(buffers, callback, opts) {
    ...
    Image.prototype.getFrame = function(frame, callback, opts) {
      ...
      if ((buffers != null ? buffers.length : void 0) === this.nBuffers) {
        ...
        try {
          if (window.convertBuffer != null) {
            for (var i = 0; i < _this.nBuffers; i++) {
              _this.frameOffsets[_this.frame].buffers.push(window.convertBuffer(blobs[i]));
            }
            return _this.getFrame(_this.frame, callback, opts);
          } else {
            buffers = [];
          ...
          for (var jj = 0; jj < this.nBuffers; jj++) {
            var ab = new ArrayBuffer(blobs[jj].length);
            var view = new Uint8Array(ab);
            for (var ii = 0; ii < blobs[jj].length; ++ii) {
              view[ii] = blobs[jj][ii];
            }
            _this.frameOffsets[frame].buffers.push(ab);
          }
          return _this.getFrame(frame, callback, opts);
        }
      }
  ...
  Tabular = (function(_super) {
    ...
    Tabular.prototype.getTableBuffer = function(row, number, callback, opts) {
      ...
      try {
        if (window.convertBuffer != null) {
          return _this.invoke(callback, opts, window.convertBuffer(blobRows))
        }
        reader = new FileReader();
        reader.row = row;
        ...
        var ab = new ArrayBuffer(blobRows.length);
        var view = new Uint8Array(ab);
        for (var ii = 0; ii < blobRows.length; ++ii) {
          view[ii] = blobRows[ii];
        }
        return _this.invoke(callback, opts, ab);
      }
    };
  ...
  window.astro.FITS.Tabular = Tabular;
  ...
  window.astro.FITS.BinaryTable = BinaryTable;
  ...
  window.astro.FITS.Decompress = Decompress;
  ...
  window.astro.FITS.CompressedImage = CompressedImage;
