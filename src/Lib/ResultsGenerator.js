...
class ResultsGenerator {
    ...
    getResultsCSV(initials, originatingSpectrumDataFilename) {
        const results = this.getResultsArray();

        initials = defaultFor(initials, "");
        if (!originatingSpectrumDataFilename)
            originatingSpectrumDataFilename = "uploaded";

        let string = "# Results generated by " + initials
            + " for file [[" + originatingSpectrumDataFilename + "]] at " + new Date().toLocaleString() + " (JSON: " + JSON.stringify(new Date()) + ") at version {{" + globalConfig.marzVersion + "}}\n";
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (i === 0) {
                    for (let k = 0; k < res.length; k++) {
                        string += ((first++ === 0) ? "" : ",  ") + res[k].name.padStart(spaces[k]);
                    }
                    string += "\n";
                    first = 0;
                }
                for (let j = 0; j < res.length; j++) {
                    string += ((first++ === 0) ? " " : ",  ") + (res[j].value == null ? "" : res[j].value.replace(/,/g,"")).padStart(spaces[j]);
                }
                string += "\n";
            }
        }
    ...
    getResultFromSpectra(spectra) {
        const result = [];
        result.push({name: "ID", value: ("" + spectra.id).padStart(4)});
        ...
        if (this.helio) {
            result.push({name: "HelioCor", value: "" + (spectra.helio == null ? 0 : this.round(spectra.helio, 7))});
        }
        if (this.cmb) {
            result.push({name: "CMBCor", value: "" + (spectra.cmb == null ? 0 : this.round(spectra.cmb, 7))});
        }
        return result;
    };
