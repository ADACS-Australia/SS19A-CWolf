 ...
 * @returns {number}
 */
export function getCMBCorrection(ra, dec, epoch, fk5) {
 ...
 * @returns {number[]}
 */
export function celestialToGalactic(ra, dec, year, fk5) {
 ...
 * @returns {number[]}
 */
export function precess(ra, dec, equinox1, equinox2, fk4) {
 ...
 * @returns {number[]}
 */
export function bprecess(ra, dec, epoch) {
    epoch = defaultFor(epoch, 2000);
 ...
 */
export function ct2lst(longitude, jd) {
 ...
 */
export function getBarycentricCorrection(dje, deq, ra, dec) {

    // Define constants
    const dc2pi = 2*Math.PI;
    const cc2pi = 2*Math.PI;
    const dc1 = 1.0;
    const dcto = 2415020.0;
    const dcjul = 36525.0; //days in Julian year
    const dcbes = 0.313;
    const dctrop = 365.24219572;  //days in tropical year (...572 insig)
    const dc1900 = 1900.0;
    const AU = 1.4959787e8;
   
    // Constants dcfel(i,k) of fast changing elements.
    const dcfel = math.matrix([[1.7400353e00, 6.2565836e00, 4.7199666e00, 1.9636505e-1, 4.1547339e00, 4.6524223e00, 4.2620486e00, 1.4740694e00],
    ...
    const dceps = math.matrix([4.093198e-1, -2.271110e-4, -2.860401e-8]);
    const ccsel = math.matrix([[1.675104E-2, 2.220221E-1, 1.589963E00, 2.994089E00, 8.155457E-1, 1.735614E00, 1.968564E00, 1.282417E00, 2.280820E00, 4.833473E-2, 5.589232E-2, 4.634443E-2, 8.997041E-3, 2.284178E-2, 4.350267E-2, 1.348204E-2, 3.106570E-2],
    ...
    const dcargs = math.matrix([[5.0974222e0, 3.9584962e0, 1.6338070e0, 2.5487111e0, 4.9255514e0, 1.3363463e0, 1.6072053e0, 1.3629480e0, 5.5657014e0, 5.0708205e0, 3.9318944e0, 4.8989497e0, 1.3097446e0, 3.5147141e0, 3.5413158e0],
    ...
    const ccamps = math.matrix([[-2.279594E-5, -3.494537E-5,  6.593466E-7,  1.140767E-5,  9.516893E-6,  7.310990E-6, -2.603449E-6, -3.228859E-6,  3.442177E-7,  8.702406E-6, -1.488378E-6, -8.043059E-6,  3.699128E-6,  2.550120E-6, -6.351059E-7],
    ...
    const ccsec3 = -7.757020E-8;
    const ccsec = math.matrix([[1.289600E-6, 3.102810E-5, 9.124190E-6, 9.793240E-7],
    [5.550147E-1, 4.035027E00, 9.990265E-1, 5.508259E00],
    [2.076942E00, 3.525565E-1, 2.622706E00, 1.559103E01]]);

    // Sidereal rates.
    const dcsld = 1.990987e-7; // sidereal rate in longitude
    const ccsgd = 1.990969e-7; // sidereal rate in mean anomaly


    // Constants used in the calculation of the lunar contribution.
    const cckm = 3.122140E-5;
    const ccmld = 2.661699E-6;
    const ccfdi = 2.399485E-7;


    // Constants dcargm(i,k) of the arguments of the perturbations of the motion
    // of the moon.
    const dcargm = math.matrix([[5.1679830e0, 5.4913150e0, 5.9598530e0],
        [  8.3286911095275e3, -7.2140632838100e3,  1.5542754389685e4]]);

    // Amplitudes ccampm(n,k) of the perturbations of the moon.
    const ccampm =  math.matrix([[ 1.097594E-1, -2.223581E-2,  1.148966E-2],
    [ 2.896773E-7,  5.083103E-8,  5.658888E-8],
    [ 5.450474E-2,  1.002548E-2,  8.249439E-3],
    [ 1.438491E-7,  -2.291823E-8,   4.063015E-8]]);

    // ccpamv(k)=a*m*dl,dt (planets), dc1mme=1-mass(earth+moon)
    const ccpamv = math.matrix([8.326827E-11, 1.843484E-11, 1.988712E-12, 1.881276E-12]);
    const dc1mme = 0.99999696;
 ...
 */
export function premat(equinox1, equinox2, fk4) {
